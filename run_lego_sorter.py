...
def run_with_retries(client: BlenderMCPClient, script: str, desc: str, attempts: int = 2, timeout: int = 300) -> bool:
...
def main():
    ...
    if os.path.exists(clear_script):
        if not run_with_retries(client, clear_script, "Scene Clearing", attempts=(1 if is_debug else 2), timeout=default_timeout):
    ...
    if os.path.exists(bucket_script):
        if not run_with_retries(client, bucket_script, "Sorting Bucket", attempts=(1 if is_debug else 2), timeout=default_timeout):
    ...
    if os.path.exists(parts_script):
        if not run_with_retries(client, parts_script, "LEGO Parts", attempts=1, timeout=default_timeout):
    ...
    if os.path.exists(physics_script):
        if not run_with_retries(client, physics_script, "Physics Animation", attempts=1, timeout=default_timeout):
    ...
    if os.path.exists(lighting_script):
        if not run_with_retries(client, lighting_script, "Lighting Setup", attempts=1, timeout=default_timeout):
    ...
    if os.path.exists(snapshot_script):
        ...
        if not run_with_retries(client, snapshot_script, "Snapshot Render", attempts=1, timeout=snap_timeout):
            print("⚠️  Snapshot rendering failed")
        print("📸 Snapshots saved to renders/snapshot*.png and renders/snapshot_ortho_*.png (camera 'SorterCam' created)")
    else:
    ...
    print("📷 A camera named 'SorterCam' should be present; latest snapshot at renders/snapshot.png")
